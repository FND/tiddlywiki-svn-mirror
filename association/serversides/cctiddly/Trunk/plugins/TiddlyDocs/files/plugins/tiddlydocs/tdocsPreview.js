...
config.macros.docPreview.handler = function(place,macroName,params,wikifier,paramString,tiddler) {
	...
	createTiddlyElement(place, "br");
	config.macros.docPreview.recurse(place, spec, 0, [0]);
}
...
//		var title = createTiddlyElement(place,"h"+level, null, null, label.join(".")+" : "+item[e].title);
	var title = createTiddlyElement(place,"h"+level, null, null, level+" : "+item[e].title);
