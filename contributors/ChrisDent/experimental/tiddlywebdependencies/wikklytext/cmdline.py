...
#
# written by fmcingvale@boodebr.org
#
...
# defaults for given types, if arg is missing
OPT_DEFAULTS = {OPT_STRING: None, OPT_BOOL: 0, OPT_LIST: [], OPT_INT: None}
...
class BasicOptParser:
	...
	def add_stropt(self, shortopt, longopt, attr, help='', default=None):
		"""Add an option taking a string argument, i.e. '-R value'.
		Defaults to None if option not used."""
		self.add_typeopt( OPT_STRING, shortopt, longopt, attr, help, default )
		
	def add_listopt(self, shortopt, longopt, attr, help='', default=None):		
		...
		self.add_typeopt( OPT_LIST, shortopt, longopt, attr, help, default )

	def add_boolopt(self, shortopt, longopt, attr, help='', default=None):
		"""Add a boolean (on/off) option, i.e. '-r'.
		Stores 1 if option used, 0 if not."""
		self.add_typeopt( OPT_BOOL, shortopt, longopt, attr, help, default )

	def add_intopt(self, shortopt, longopt, attr, help='', default=None):
		"""Add am integer option, i.e. '-r NN'.
		Stores None if option not used."""
		self.add_typeopt( OPT_INT, shortopt, longopt, attr, help, default )		
	...
	def process(self, argv):
		...
		Typically you'll call this as:
			op.process(sys.argv[1:])
		"""
	...
	def show_usage(self, tail='arg, ...'):
		"""Print usage/help information"""

		print "%s - %s\n\nUsage: %s [options] %s\n" % \
			  (self.prog_name,self.prog_info,self.prog_name,tail)

		for otype, s, l, attr, help, default in self.opts:
			
			if otype in [OPT_STRING,OPT_INT]:
				arg = ' arg'
			elif otype == OPT_LIST:
				arg = ' arg,...' # comma-separated, NO spaces
			else:
				arg = ''
			...
			hs = hs + '\n\t' + '(Default=%s)' % (default or OPT_DEFAULTS[otype])
			
			print hs

	# -*- internal API below -*-
	
	def add_typeopt(self, otype, shortopt, longopt, attr, help, default):
		if shortopt != '' and len(shortopt) != 1:
			raise "shortopt must be a single char, or ''"
				
		tup = (otype, shortopt, longopt, attr, help, default)
	...
	def make_getopt_args(self):
		...
		for otype, shortopt, longopt, attr, help, default in self.opts:
			if len(shortopt):
...
class BasicOptDataVal:
	def __init__(self,opts):		
		for otype,s,l,attr,h,default in opts:
			...
			# set user-specified default, or use builtin default value
			setattr(self, attr, default or OPT_DEFAULTS[otype])
