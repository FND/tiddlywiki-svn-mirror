...
The templateTiddlers macro finds a set of tiddlers and renders them once each through a template. The template can contain additional calls to the macro to allow for e.g. looping inside a template (think RSS items). It needs to be able to support recursion, so that sub-templates make sense.
...
config.macros.templateTiddlers.doc = {
	desc: "The templateTiddlers macro finds a set of tiddlers and renders them once each through a template. The template can contain additional calls to the macro to allow for e.g. looping inside a template (think RSS items). It needs to be able to support recursion, so that sub-templates make sense.",
	usage: '<<templateTiddlers template filterString>>',
	params: [
		{
			name: "template",
