...
FeedListManager.prototype.add = function(uri, name, type, params) {
	...
	if(!name)
		name = 'anon';
	var uriObj = {'name':name, 'uri':uri, 'type':type, 'params':params, 'callCount':0, 'lastCall':null};
...
FeedListManager.prototype.remove = function(uri) {
	...
	var feedlist = this;
	if(pos != null)
		feedlist.uris.splice(pos,1);
	if(feedlist.currentPosition >= feedlist.uris.length) // it is possible for currentPosition to equal the list length due to removal of items
