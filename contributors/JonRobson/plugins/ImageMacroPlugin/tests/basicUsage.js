...
/*test("image foo alt:broken altImage:foo.gif", function() {
	var place = $("#imageTestArea")[0];
	wikify("<<image SVGExample.svg alt:brokenx altImage:foo.gif>>", place, null, store.getTiddler("foo"));
	var img = $("img", "#imageTestArea");
	same(img.length, 1);
	same(img.attr("src"), "foo.gif", "make sure foo.gif is present in test folder");
});*/
