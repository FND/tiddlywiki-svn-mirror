...
''Revision history:''
* v1.0.1 (Apr 27, 2008)
** Fixed bug, empty only_on_tags should allow all tiddlers could be commented (bug reported by MilchFlasche), except the tiddlers tagged with 'not_on_tags' (also shadowed).<br>(in general, the array only_on_tags would contain one element 'comment' at least, not really empty)
* v1.0.0 (Feb 3, 2008)
...
* v0.7.0 (Nov 09, 2006)
** Minor changes, more easier to be translated
* v0.6.0 (Nov 09, 2006)
** Runs compatibly with TW 2.1.0+
* v0.5.0 (Jun 15, 2006)
** Fixed bug for feature of CommentEditTemplate (bug reported by MilchFlasche)
** Fixed bug in redefined TiddlyWiki.prototype.saveTiddler
* v0.4.0 (Jun 03, 2006) Added CommentEditTemplate
* v0.3.0 (Jun 01, 2006) Some minor changes for readOnly mode
* v0.2.0 (Apr 04, 2006) Fixed bug for only_on_tags
...
//config.CommentPlugin.only_on_tags.pushUnique(CPlingo.comments);
...
config.macros.newCommentLink = {
	...
	handler: function(place,macroName,params,wikifier,paramString,tiddler) {
		...
		if(tiddler && store.tiddlerExists(tiddler.title) && !readOnly && (!window.zw || zw.loggedIn || zw.anonEdit)) {
			if(tiddler.tags.containsAny(config.CommentPlugin.not_on_tags) || !(tiddler.tags.containsAny(config.CommentPlugin.only_on_tags) || config.CommentPlugin.only_on_tags == CPlingo.comments))
				return;
