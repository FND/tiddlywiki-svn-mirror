...
!StyleSheet
.tagg { margin-right: 8px; cursor: pointer; }
.tag0 { font-size: 80%; }
.tag1 { font-size: 100%; }
.tag2 { font-size: 115%; }
.tag3 { font-size: 130%; }
...
(function($) {
  ...
  var macro = config.macros.tagCloud = {
    ...
    handler: function(place,macroName,params,wikifier,paramString,tiddler) {
      ...
      var includeTag = getParam(macroParams, "includeTag");

      var tiddlers = includeTag ? _.select(store.getTiddlers(), function(tiddler) {
        return tiddler.isTagged(includeTag);
      }) : store.getTiddlers();
      var tagRecords = [];
      _.each(tiddlers, function(tiddler) {
        _.each(tiddler.tags, function(tag) {
          if (/^user_/.test(tag)) {
            ...
            log("tid", tiddler, "Tag", tag, "tagRec " + tagRecord.tag + " " + tagRecord.count);
            tagRecord.count++;
          }
        });
      });
      ...
      $.each(tagRecords, function(i, tagRecord) {
        $("<a/>").html(tagRecord.tag.replace(/user_/,""))
