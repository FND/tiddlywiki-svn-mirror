...
config.macros.importWikispacesMessages.getTopicListCallback = function(context,userParams)
{
	if(context.status) {
		...
		for(var i=0; i<tiddlers.length; i++) {
			...
			context.adaptor.getMessageList(tiddler.fields['server.topic_id'],context,{"topic":tiddler.title},config.macros.importWikispacesMessages.getMessageListCallback);
		}
	} else {
