...
//		context.callback(context,context.userParams);
	...
	context.adaptor.getTiddlerTags(context.tiddler.title,context,context.userParams,context.callback);

};

WikispacesSoapAdaptor.prototype.getTiddlerTags = function(title,context,userParams,callback)
{
	//console.log('adaptor getTiddlertags',arguments)
	context = this.setContext(context,userParams,callback);
	return this.complete(context,WikispacesSoapAdaptor.getTiddlerTagsComplete);
};

WikispacesSoapAdaptor.getTiddlerTagsComplete = function(context,userParams)
{
	//console.log('adaptor getTiddlertagscomplete',arguments)
	var uri = WikispacesSoapAdaptor.SoapUri(context,'%0tag/api');
	var pl = new SOAPClientParameters();
	pl.add('session',context.sessionToken);
	pl.add('pageId',context.pageId);
	SOAPClient.invoke(uri,'listTagsForPage',pl,true,WikispacesSoapAdaptor.getTiddlerTagsCallback,context);
	return true;
};

WikispacesSoapAdaptor.getTiddlerTagsCallback = function(r,x,context)
{
	...
	if(status) {
		try {
			...
			for (var m=0; m<tags.length; m++){
				tiddler.tags.push(gev(tags,m,'name'));
			}			
			context.tiddler = tiddler;
...
fnLog('createTiddlerCallback');
	...
	if(status) {
		...
		try {
			var p = x.getElementsByTagName('page');
			if (!p.length)
				p = x.getElementsByTagName('updatedPage');
			//console.log(p)
			var tiddler = context.tiddler;
			var i = 0;
			tiddler.modified = WikispacesSoapAdaptor.dateFromTimestamp(gev(p,i,'date_created'));
...
//#fnLog('uri:'+uri);
	...
	pl.add('spaceId',context.workspaceId);
	pl.add('page',page);
	SOAPClient.invoke(uri,'updatePage',pl,true,WikispacesSoapAdaptor.createTiddlerCallback,context);
	return true;
};
...
WikispacesSoapAdaptor.prototype.updateTags = function(context)
{
	...
	pl.add('session',context.sessionToken);
	pl.add('pageId',context.pageId);
	SOAPClient.invoke(uri,'listTagsForPage',pl,true,WikispacesSoapAdaptor.updateTagsCallback1,context);
	return true;
};

WikispacesSoapAdaptor.updateTagsCallback1 = function(r,x,context)
{
	//console.log('updateTagsCallback1',arguments);
	var status = r instanceof Error ? false : true;
	context.status = false;
	context.statusText = WikispacesSoapAdaptor.errorInFunctionMessage.format(['updateTagsCallback1']);
	function gev(p,i,n) {
		try {
			return p[i].getElementsByTagName(n)[0].childNodes[0].nodeValue;
		} catch(ex) {
