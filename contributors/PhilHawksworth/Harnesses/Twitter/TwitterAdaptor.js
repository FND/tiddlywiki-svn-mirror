...
|''CodeRepository:''|http://svn.tiddlywiki.org/Trunk/contributors/PhilHawksworth/adaptors/TwitterAdaptor.js |
|''Version:''|0.1.6|
...
//{{{
if(!version.extensions.TwitterAdaptor) {
version.extensions.TwitterAdaptor = {installed:true};
...
TwitterAdaptor.NotLoadedError = "RSS file has not been loaded";
TwitterAdaptor.serverType = 'rss';

TwitterAdaptor.prototype.setContext = function(context,userParams,callback)
{
	if(!context) context = {};
	...
	context.host = TwitterAdaptor.fullHostName(context.host);
	if(!context.workspace)
		context.workspace = this.workspace;
	return context;
};

TwitterAdaptor.fullHostName = function(host)
{
	if(!host)
		return '';
	if(!host.match(/:\/\//))
...
TwitterAdaptor.prototype.openHost = function(host,context,userParams,callback)
{
	this.host = TwitterAdaptor.fullHostName(host);
...
TwitterAdaptor.loadRssCallback = function(status,context,responseText,url,xhr)
{
	...
	if(!status) {
		...
		try {
			...
			context.tiddlers = TwitterAdaptor.rssToTiddlers(responseText,context.rssUseRawDescription);
			/*if(context.filter) {
...
TwitterAdaptor.prototype.openWorkspace = function(workspace,context,userParams,callback)
{
...
TwitterAdaptor.prototype.getTiddlerList = function(context,userParams,callback,filter,username,password)
{
...
//#displayMessage("h:"+context.host);
	...
	context.complete = TwitterAdaptor.getTiddlerListComplete;
	if(context.tiddlers) {
		var ret = context.complete(context,context.userParams);
	} else {
		// ret = loadRemoteFile(context.host,TwitterAdaptor.loadRssCallback,context);
		
		console.log("loading "+ context.host +" with the credentials: "+username+":"+password);
		
		ret = doHttp("GET",context.host,null,null,username,password,TwitterAdaptor.loadRssCallbackk,context,null);	
...
TwitterAdaptor.getTiddlerListComplete = function(context,userParams)
{
...
TwitterAdaptor.prototype.generateTiddlerInfo = function(tiddler)
{
	var info = {};
...
//#displayMessage("getTiddler:"+title);
	context = this.setContext(context,userParams,callback);
	context.title = title;
	context.complete = TwitterAdaptor.getTiddlerComplete;
	return context.tiddlers ? 
		context.complete(context,context.userParams) :
		loadRemoteFile(context.host,TwitterAdaptor.loadRssCallback,context);
};

TwitterAdaptor.getTiddlerComplete = function(context,userParams)
{
...
TwitterAdaptor.prototype.close = function()
{
};

TwitterAdaptor.rssToTiddlers = function(rss,useRawDescription)
{
