...
TiddlyWiki.prototype.loadShadowsFromDiv = function(src,idPrefix,noUpdate)
{
...
TiddlyWiki.prototype.loadFromDiv = function(src,idPrefix,noUpdate,shadow)
{
	this.idPrefix = idPrefix;
	var storeElem = (typeof src == "string") ? document.getElementById(src) : src;
	if(!storeElem)
		return;
	if(shadow){
		var tiddlers = this.getLoader().loadTiddlers(this,storeElem.childNodes,shadow);
	}
	else {
		var ts = $(storeElem).find('div.tiddler').get();
		var tiddlers = this.getLoader().loadTiddlers(this,ts,shadow);	
	}
