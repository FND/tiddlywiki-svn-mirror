...
TODO:
Split into two plugins, one to login and another to manage the behaviour
Check for the presence of the ribbit login plugin and extend it
Split this ino a Voicemail collector plugin
...
(function($) {
    ...
    config.macros.RibbitLogin = {
        ...
        handler: function(place, macroName, params, wikifier, paramString, tiddler){
            ...
            if(Ribbit.userId!=null){
                ...
                var t = store.createTiddler("Ribbit Login");
                
                store.saveTiddler(t)
            }
            /**
            if(Ribbit.userId==null&&getParam(macroParams,"secretKey")!=null){
                //user wants to do a 3LeggedAuthentication
                log("login on 3Legged auth");
                //loginPopup(place, macroParams);
    ...
    //note this is _exactly_ the same behaviour as Login3Leg...
    function loginPopup(place,macroParams){
        Ribbit.getAuthenticatedUserInPopup(getMessages(), Popup);
    }
