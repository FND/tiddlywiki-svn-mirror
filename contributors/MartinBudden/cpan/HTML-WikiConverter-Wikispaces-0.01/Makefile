# This Makefile is for the HTML::WikiConverter::Wikispaces extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/HTML/WikiConverter/Wikispaces.pm]
#     AUTHOR => q[Martin Budden <mjbudden@gmail.com>]
#     NAME => q[HTML::WikiConverter::Wikispaces]
#     PL_FILES => {  }
#     PREREQ_PM => { Test::More=>q[0], HTML::WikiConverter=>q[0.52] }
#     VERSION_FROM => q[lib/HTML/WikiConverter/Wikispaces.pm]
#     clean => { FILES=>q[HTML-WikiConverter-Wikispaces-*] }
...
NAME_SYM = HTML_WikiConverter_Wikispaces
VERSION = 0.01
...
FULLEXT = HTML/WikiConverter/Wikispaces
BASEEXT = Wikispaces
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/HTML/WikiConverter/Wikispaces.pm
...
TO_INST_PM = lib/HTML/WikiConverter/.DS_Store \
	lib/HTML/WikiConverter/Wikispaces.pm \
	towiki.pl \
	towikim.pl

PM_TO_BLIB = lib/HTML/WikiConverter/Wikispaces.pm \
	blib/lib/HTML/WikiConverter/Wikispaces.pm \
	towikim.pl \
	$(INST_LIB)/HTML/WikiConverter/towikim.pl \
	lib/HTML/WikiConverter/.DS_Store \
	blib/lib/HTML/WikiConverter/.DS_Store \
	towiki.pl \
	$(INST_LIB)/HTML/WikiConverter/towiki.pl
...
DISTNAME = HTML-WikiConverter-Wikispaces
DISTVNAME = HTML-WikiConverter-Wikispaces-0.01
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW)\
	  lib/HTML/WikiConverter/Wikispaces.pm $(INST_MAN3DIR)/HTML::WikiConverter::Wikispaces.$(MAN3EXT) 
...
clean :: clean_subdirs
	-$(RM_RF) HTML-WikiConverter-Wikispaces-* ./blib $(MAKE_APERL_FILE) $(INST_ARCHAUTODIR)/extralibs.all $(INST_ARCHAUTODIR)/extralibs.ld perlmain.c tmon.out mon.out so_locations pm_to_blib *$(OBJ_EXT) *$(LIB_EXT) perl.exe perl perl$(EXE_EXT) $(BOOTSTRAP) $(BASEEXT).bso $(BASEEXT).def lib$(BASEEXT).def $(BASEEXT).exp $(BASEEXT).x core core.*perl.*.? *perl.core core.[0-9] core.[0-9][0-9] core.[0-9][0-9][0-9] core.[0-9][0-9][0-9][0-9] core.[0-9][0-9][0-9][0-9][0-9]
...
realclean purge ::  clean realclean_subdirs
	...
	$(RM_RF) $(DISTVNAME)
	$(RM_F)  $(INST_LIB)/HTML/WikiConverter/towikim.pl blib/lib/HTML/WikiConverter/Wikispaces.pm $(INST_LIB)/HTML/WikiConverter/towiki.pl $(MAKEFILE_OLD) $(FIRST_MAKEFILE) blib/lib/HTML/WikiConverter/.DS_Store
...
metafile :
	...
	$(NOECHO) $(ECHO) 'name:         HTML-WikiConverter-Wikispaces' >> META.yml
	$(NOECHO) $(ECHO) 'version:      0.01' >> META.yml
	$(NOECHO) $(ECHO) 'version_from: lib/HTML/WikiConverter/Wikispaces.pm' >> META.yml
	$(NOECHO) $(ECHO) 'installdirs:  site' >> META.yml
	$(NOECHO) $(ECHO) 'requires:' >> META.yml
	$(NOECHO) $(ECHO) '    HTML::WikiConverter:           0.52' >> META.yml
...
ppd:
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Convert HTML to Wikispaces markup</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Martin Budden &lt;mjbudden@gmail.com&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="HTML-WikiConverter" VERSION="0,52,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib: $(TO_INST_PM)
	$(NOECHO) $(PERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')'\
	  lib/HTML/WikiConverter/Wikispaces.pm blib/lib/HTML/WikiConverter/Wikispaces.pm \
	  towikim.pl $(INST_LIB)/HTML/WikiConverter/towikim.pl \
	  lib/HTML/WikiConverter/.DS_Store blib/lib/HTML/WikiConverter/.DS_Store \
	  towiki.pl $(INST_LIB)/HTML/WikiConverter/towiki.pl 
	$(NOECHO) $(TOUCH) $@
